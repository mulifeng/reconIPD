  ...
  sidebarLayout(
    ...
    mainPanel(
      ...
      tabsetPanel(
        ...
        tabPanel("KMdata", textOutput("KMdata")),
        tabPanel("IPD", textOutput("IPD")),
...
server <- function(input, output) {
    ...
    digizeit <- reactive({
      read.xlsx(paste0(input$path, input$digisurvfile), sheet = 1)
    })
    ...
    pub.risk <- reactive({
      read.xlsx(paste0(input$path, input$nriskfile), sheet = 1)
    })
    ...
    KMdata <- reactive({
      ...
      as.data.frame(matrix(c(temp()$t.S, temp()$n.hat[1:n.t], temp()$d, temp()$cen), ncol=4, byrow=F))
    ...
    IPD <- reactive({
      ...
      as.data.frame(matrix(c(temp()$t.IPD, temp()$event.IPD, temp()$arm), ncol=3, byrow=F))
  ...
  output$KMdata <- renderText({
    
    KMdata
    
  })
  
  output$IPD <- renderText({
  ...
  output$KMplot <- renderPlot({
    ...
    plot(KM.est(), ylim=c(0, 1), yaxt = "n", xlim = c(0, 10),
